...
void proxEstado (Celda ***ma, Celda ***ma2, int cantCelReal, int t){// el t es el clock
    ...
    for(int i = 0; i<cantCelReal; i++){
        for(int j = 0; j<cantCelReal; j++){
            ...
            vecino.naranja = 0;
            vecino.verde = 0;
            if((*ma)[i][j].Estado.verde == 1){ // si esta sano
                if((i == 0 || i == cantCeldas) && (j == 0 || j == cantCeldas)){ //Las cuatro esquinas
                    if(j == 0){
                        vecino.rojo = (*ma)[i][j+1].Estado.rojo + vecino.rojo;
                        vecino.rojo = (*ma)[i][j+2].Estado.rojo + vecino.rojo;
                    }
                    else {
                        vecino.rojo = (*ma)[i][j-1].Estado.rojo + vecino.rojo;
                        vecino.rojo = (*ma)[i][j-2].Estado.rojo + vecino.rojo;
                    ...
                    if(i == 0){
                        if(j == 0){
                            vecino.rojo = (*ma)[i+1][j+1].Estado.rojo + vecino.rojo;
                        }
                        else {
                            vecino.rojo = (*ma)[i+1][j-1].Estado.rojo + vecino.rojo;
                        }
                        vecino.rojo = (*ma)[i+1][j].Estado.rojo + vecino.rojo;
                        vecino.rojo = (*ma)[i+2][j].Estado.rojo + vecino.rojo;
                    }
                    else {
                        if(j == 0){
                            vecino.rojo = (*ma)[i-1][j+1].Estado.rojo + vecino.rojo;
        ...
        //---------------------------------------------------------------------------
                ...
                (*ma2)[i][j].Estado.verde = 1;
                if((*ma)[i][j].Herida == 1 && t != 0){ //  t!= 0 por si en la primera iteración ya tenia un aherida para no curarla porque no paso 1 se(*ma)na
                    // supongo de que ya paso 1 semana de que tuvo la herida asi que ya se sanó
                    (*ma2)[i][j].Herida = 0;
                }
                if(aniosVida((*ma)[i][j].Edad) <= 3 ){//arol joven
                    ...
                    ran = rand() % 101;
                    if (ran <= pContagio){
                        (*ma2)[i][j].Estado.verde = 0;
                        (*ma)[i][j].Estado.verde = 0;

                        (*ma2)[i][j].Estado.naranja = 1;
                        (*ma2)[i][j].Enfermo ++;// aumento 1 porque va a pasar una semana en el proximo clock
                    }
                }
                if(aniosVida((*ma)[i][j].Edad) <= 35 && aniosVida((*ma)[i][j].Edad) > 3){
    ...
    //--------------------------------------------------------------------------------------------------------------
            if((*ma)[i][j].Estado.naranja == 1){ //infectado con esporas
                (*ma2)[i][j].Estado.naranja = 1;
                if((*ma)[i][j].Enfermo < 3){
                    (*ma2)[i][j].Enfermo ++;// aumento 1
               }
               else {//de infectado con esporas a enfermo con síntomas
                   (*ma2)[i][j].Estado.naranja = 0;
                   (*ma)[i][j].Estado.naranja = 0;

                   (*ma2)[i][j].Estado.rojo = 1;
                   (*ma2)[i][j].Enfermo ++;// aumento 1
               }
            }
            if((*ma)[i][j].Estado.rojo == 1){//enfermo con síntomas
                (*ma2)[i][j].Estado.rojo = 1;
                ran = rand() % 101;// random de 0 a 100
                if(ran <= 85){// 85% de que esto ocurra paso de enfermo con síntomas a enfermo con tratamiento
                    (*ma2)[i][j].Estado.rojo = 0;
                    (*ma)[i][j].Estado.rojo = 0;

                    (*ma2)[i][j].Estado.azul = 1;
                }
                (*ma2)[i][j].Enfermo ++;// aumento 1
            }
            if((*ma)[i][j].Estado.azul == 1){//enfermo con tratamiento antifúngico
                (*ma2)[i][j].Estado.azul = 1;
                ran = rand() % 101;// random de 0 a 100
                if(aniosVida((*ma)[i][j].Edad)<= 3){ //arbol joven
                    if(ran <= 3){//si tiene resistencia al hongo

                        if((*ma2)[i][j].Enfermo <= 8){
                            (*ma2)[i][j].Estado.azul = 0;
                            (*ma)[i][j].Estado.azul = 0;

                            (*ma2)[i][j].Estado.verde = 1;//pasa a sano con la misma edad
                            (*ma2)[i][j].Enfermo = 0;//0 porque ya no está más enfermo en el proximo clock
                        }
                        else{//se poda el arbol porque ya pasaron 8 o más semanas del contagio
                            (*ma2)[i][j].Estado.azul = 0;
                            (*ma)[i][j].Estado.azul = 0;                

                            (*ma2)[i][j].Estado.blanco = 1;//podado
                            (*ma2)[i][j].Enfermo = 0; //0 porque ya no esta más enfermo en el proximo clock
                            (*ma2)[i][j].Podado = 1; //al proximo clock va a ser 1 semana de ser podado
                        }
                    }
                    else{//en esta semana no tiene resistencia
                        (*ma2)[i][j].Enfermo ++; //pasa el tiempo nomas
                    }
                }
                if(aniosVida((*ma)[i][j].Edad) <= 35 && aniosVida((*ma)[i][j].Edad) > 3){//adulto
                ...
                if(aniosVida((*ma)[i][j].Edad) > 35){//viejo
            ...
            if((*ma)[i][j].Estado.blanco == 1){// arbol podado
                (*ma2)[i][j].Estado.blanco = 1;
                if((*ma2)[i][j].Podado == 12){
                    //de podado a sano
                    (*ma2)[i][j].Estado.blanco = 0;
                    (*ma)[i][j].Estado.blanco = 0;

                    (*ma2)[i][j].Estado.verde = 1;
                    (*ma2)[i][j].Podado = 0;//para reiniciar la variable nomas
                }
                else{
                    (*ma2)[i][j].Podado ++;
                }
...
main(int argc, char **argv){
    ...
    int cantCelReal = 2000;//para probar nomas despues agregar los valores reales
    ...
    for(int i = 0; i < 10; i ++){
        ...
        for (t = 0; t<1200; t++){
            proxEstado(&matris, &matris2,cantCelReal, t);//30 para matriz de 30x30 ver otros ejemplos
